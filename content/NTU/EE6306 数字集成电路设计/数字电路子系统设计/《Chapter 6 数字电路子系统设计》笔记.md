...
![[阵列乘法器.png]]
整列乘法器在位串行乘法器的基础上，用整列取代了移位，使得所有位的乘数和被乘数能同时进行与计算。随后进行三次部分积的加和即可得到结果。
...
![[关键路径和传播延时.png]]
M-1 是第一行 carry 只传 M-1 次，从最后一个 sum 下去。N-2 是因为第一行不用传 carry（因为输入进位是 0），最后一行不用传，因为直接输出 sum。所以 sum 是 N-1，因为 N 行只用将部分积加和 N-1 次就能得出结果了。
