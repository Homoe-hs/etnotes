...
波尔兹曼关系就是用费米势去表示热平衡载流子浓度。[[费米势]]定义为，
![[费米势定义式]]

>[! note]
>这里可以辅助理解外在电压是如何影响费米能级变化的。你施加正电压导致费米势增加，从而导致费米能级的降低。
...
![[用准费米势表示 P 型基极中电子总电流密度]]
[[用准费米势表示 P 型基极中电子总电流密度推导过程]]
...
有集电极电流为，![[BJT 紧凑模型集电极电流计算式]]
$J_{C_{0}}$ 定义为饱和集电极电流密度，![[集电极饱和电流密度]]
基区古梅尔数定义为，![[基极古梅尔数计算式]]
>[! note] 
...
和发射极古梅尔数，
![[发射极古梅尔数定义式]]
$I_{C}$ 取决于基极古梅尔数，$I_{B}$ 取决于发射极古梅尔数。
...
>[! summary]
>这里总结一下实际电流和理想电流出现偏差的原因。原因的具体内容在 [[《Chapter 1 硅 BJT》笔记#3. 非理想 BTJ 行为]]有叙述。
>1. 小注入时，$I_{B}$ 高于理想值
>	- 基极中的复合-产生电流
>	- 基极中的隧穿电流
>2. 大注入时，$I_{B}$ 低于理想值
>	- 发射极和基极的寄生电阻 $r_{e}$、$r_{bi}$ 和 $r_{bx}$
>3. 大注入时，$I_{C}$ 低于理想值
>	- 发射极和基极的寄生电阻
>	- 基极电导调制效应
...
在看这里之前，需要先了解[[《Chapter 1 硅 BJT》笔记#4.1. 基本直流 Ebers-Moll 模型]]
...
![[200 文献笔记/线下课程/NTU/2024 秋/EE6604 半导体器件高级主题/硅 BJT/BJT 基极传输因子定义式]]
...
$\alpha_{F}=1$ 也就是 $BV_{CEO}$ 电路雪崩击穿的条件。
...
扩散电容 $C_{DE}$ 是基极-发射极正向偏置所引起的**所有少数载流子**导致的寄生电容。 
$$
...
定义为发射极和集电极短路时（$R_{L} = 0$），共发射极小信号电流增益降至单位 1 时的渡越频率。因此，它也称为单位电流增益频率。$f_{T}$ 描述晶体管用作放大器时的最大有用频率，超出截止频率后 BJT 不在有放大作用。
...
>是[[电导调制效应|基极电导率调制效应]]引起的（可以结合古梅尔图来看）。
...
>[! note]
>这里可以看出，当扩散电容越大，$Re(Z_{out})$ 越大，电压增益越小。
