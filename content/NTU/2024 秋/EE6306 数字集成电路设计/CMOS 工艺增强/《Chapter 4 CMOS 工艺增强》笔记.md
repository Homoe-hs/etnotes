...
同样的，较薄的氧化层厚度能带来更高的导通电流（参考[[长沟道 MOS 器件线性区漏极电流计算式]]），但是在高电压下却容易击穿。
...
SOI 技术就是将原本的硅基底换成绝缘体（见第一个结构到第二个结构的转变），例如 $\text{SiO}_{2}$ 和蓝宝石（Sapphire）。
