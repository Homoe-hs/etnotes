...
同步失败，或者说同步器进入亚稳态，指随着两个输入信号之间的时间差 $\Delta t$ 减少，越来越难以判断哪个信号先来。由此进入了亚稳态。
...
如果输入信号在触发器的[[孔径时间]] aperture time （ta） 期间转换，则 FF1 可能会进入亚稳态。给定转换处于孔径时间内的概率, 即孔径时间所占的时间比例。
...
> [!note] 等待时间
> 如果继续增加级数，相当于增加了 $t_{w}$，即给亚稳态增加了衰减时间。
> - **第一级**：决定系统进入亚稳态的**初始概率**。
> - **后续级**：通过延长衰减时间**指数级降低失败概率**。
> 所以总的等待时间 
> $$
> t_{w} = (n-1)(T_{cy}-t_{s}-t_{dCQ})
> $$
> $n$ 为级联触发器总级数；$T_{cy}$ 为时钟周期；$t_{s}$ 为 FF 建立时间；$t_{dCQ}$ 为触发器时钟到输出延迟。
