...
根据相联方式，可以分为：
- 全相联：相当于所有缓存行全随机；
- 组相联：相当于在一个 set 内，缓存行随机（或者按照替换规则）替换；
- 直接相联：点对点固定位置替换。
...
$$
我们想要 $W$ 相对于 $S_{p}$ 有最小值，于是我们对其求导，
$$
\dfrac{ \partial W }{ \partial S_{p} } = \dfrac{1}{2}-\dfrac{S_{s}}{S_{p}^{2}} 
$$
令 $\dfrac{ \partial W }{ \partial S_{p} }=0$，有 $S_{p}=\sqrt{ 2S_{s} }$ ，此时 $W$ 有最小值。
...
设逻辑地址序列为 $A_{0},A_{1},\dots,A_{L-1}$。设 $A_{i}$ 为某一时刻引用的逻辑地址。设 $A_{i+d}$ 为下一个引用, 其中 $d$ 表示引用和下一个参考点之间的距离。

1. 当 $d$ 相对于 $S_{p}$ 较小，相当于使得 $A_{i}$ 和 $A_{i+1}$ 位于同一页内。命中 $A_{i}$ 后再命中 $A_{i+1}$ 的概率**随页大小的增加而增大**。 
2. 当 $d$ 相对于 $S_{p}$ 较大, 但 $A_{i+d}$ 关联着一组被频繁引用的字, 我们希望 $A_{i+d}$ 能位于某个同样在主内存内但是 $P'\neq P$ 的页（就是两个数据都在主存，但是在不同页）。命中率会随着主内存能存储的页面数量增加而提高; 因此, 它往往**会随着页面大小的增大而降低**。

所以呈现一个先升后降的情况。

![[页大小对命中率的影响.png]]

仿真研究表明，在大型系统中，产生最佳命中率的 $S_{p}$ 值可能远大于最佳页面大小，$S_{p}=\sqrt{ 2S_{s} }$。从系统速度性能的角度来看，更高的命中率总是优于最佳 $S_{p}$。
...
**逻辑地址 logic address**：是由分段程序生成的地址。它与虚拟地址类似, 不同之处在于地址空间关联的是可变长度的段, 而非固定长度的页。
