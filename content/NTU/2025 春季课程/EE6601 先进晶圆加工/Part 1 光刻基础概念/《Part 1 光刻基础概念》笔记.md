...
相移掩膜在掩膜的基础上，通过使用不同材料厚度的图层（相位层）来优化掩膜的[[临近效应]]导致的光波扩散。具体来说，相位层通过180°相位差抵消邻近衍射效应，使得两个区域之间不再形成衍射，提高成像对比度。

最直接的效果就是减小了系统常数 $k$。

### 4.2.2. 光学邻近校正 Optical Proximity Correction (OPC)

最初的掩膜就是图样的形状，但是受到衍射等影响导致光刻出的图样和实际图样不符。 OPC 就是通过仿真等方法来修正掩膜，使得光刻后的结果接近图样。
...
通过具有更高折射率的浸没液，光的出射角度更小，总体上来说是 DOF 增加了，光刻的最小线宽没有变。新的 DOF 计算式如下，
$$
\sigma = \dfrac{k_{2}\lambda}{\text{NA}^{2}}(\dfrac{n_{\text{water}}}{1})
$$
> [! question]
> 总感觉公式有问题
