...
Diffie-Hellman 算法的核心就是找到这样一个素数 $p$ 的原根 $g$。用户 A 选择一个私钥 $X_{A}<p$，计算出公钥 $Y_{A}=g^{X_{A}}\mod p$，用户 B 选择一个私钥 $X_{B}<p$，计算出公钥 $Y_{B}=g^{X_{B}}\mod p$，然后相互发送公钥。用户 A 又生成一个密钥 $K_{A}=Y_{B}^{X_{A}}\mod p$，用户 B 又生成一个密钥 $K_{B}=Y_{A}^{X_{B}}\mod p$，这里的 $K_A = K_{B}$。这里用户 A 和 B 就拥有了同样的私钥 K。
